...
- **Provide content at the highest available resolution** — include the full HLS resolution range. AVFoundation selects the right resolution automatically. Without a full range, content looks low quality when streamed to a higher-resolution device (e.g., 720p → 4K TV).
...
- **Always use as a noun** — ✓ "Use AirPlay to listen" — ✗ "You can AirPlay with [App]"
- Use terms like *works with*, *use*, *supports*, *compatible* — ✓ "[App] is compatible with AirPlay" — ✗ "AirPlay-enabled" as an adjective.
